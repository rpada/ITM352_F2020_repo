...
app.post("/login", function (request, response) {
    ...
    if (user_data[user_name_from_form] != undefined) {
        ...
        if (password_from_form == password_on_file) {
            ...
            if (typeof request.session.lastLogin != 'undefined') {
                var msg = `You last logged in at ${request.session.lastLogin}`;
            ...
            request.session.lastLogin = now.toString() + ' ' + now.getTime();
            request.session.Session["lastLogin"] = now.getDate() + ' ' + now.getTime();
